...
def render_main_window(self):
    ...
    self.plot_container = Frame(self.window, bg="white", padx=10)
    # añade el gráfico de matplotlib a la interfaz de tkinter
    FigureCanvasTkAgg(self.fig, self.plot_container).get_tk_widget().grid(row=0, column=0, columnspan=4)
    Label(self.plot_container, bg="white", text="Clase: ").grid(row=1, column=1, sticky="e")
    self.input_class = ttk.Combobox(self.plot_container)
    self.input_class["values"] = list(range(0, 100))
    self.input_class.set(0)

    self.plot_container.grid(row=1, column=0, columnspan=6, sticky="we")
    self.input_class.grid(row=1, column=2, sticky="w")
    ...
    self.left_container.grid(row=0, column=0, columnspan=2, sticky="we")
    self.center_container.grid(row=0, column=3, columnspan=2, sticky="we")
    self.right_container.grid(row=0, column=5, sticky="nswe")
